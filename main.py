...
def parse_args():
    ...
    args.add_argument("-h_gat", "--nheads_GAT", type=int, nargs='+',
                      default=[8], help="Multihead attention SpGAT")
...
def load_data(args):
    # train_data,  entity2id, relation2id, headTailSelector = build_data(
    #     args.data, is_unweigted=False, directed=True)
    train_data, validation_data, test_data, entity2id, relation2id, headTailSelector = build_data(
        args.data, is_unweigted=False, directed=True)
    ...
    # corpus = Corpus(args, train_data,  entity2id, relation2id, headTailSelector,
    #                 args.batch_size_gat, args.valid_invalid_ratio_gat, args.get_2hop)
...
def train_gat(args):
    ...
    model_gat = HAN(num_meta_paths=11,
                   embedd_dim=args.embedding_size,
                   hidden_size=args.hidden_units,
                   # relation_embed= relation_embeddings,
                   entity_out_dim=args.entity_out_dim,
                   num_heads=args.nheads_GAT,
                   dropout=args.drop_GAT,
                   alpha=args.alpha
                )
    if CUDA:
        model_gat.cuda()
    ...
    for epoch in range(args.epochs_gat):
        ...
        for iters in range(num_iters_per_epoch):
            ...
            if CUDA:
                ...
                # train_adj = Corpus_.train_adj.cuda()
                # train_adj = Variable(train_adj)
                global relation_embeddings , entity_embeddings
                relation_embedding = relation_embeddings.cuda()
                entity_embedding = entity_embeddings.cuda()
                relation_embeddings , entity_embeddings = Variable(relation_embedding),  Variable(entity_embedding)
            ...
            #     Corpus_, Corpus_.train_adj, train_indices)
            relation_embeddings = model_gat.init_relation_embedding()
            entity_embed, relation_embed = model_gat.forward(Corpus_.train_adj,entity_embeddings, relation_embeddings)
